...
use crate::{models::news::{comment::PostComment, NewsId}, utils::jwt::verify_jwt};
...
pub async fn create_comment(
  ...
  match verify_jwt(&req) {
    Some(reader_id) => {
      let query = format!("INSERT INTO {} (article_id, reader_id, content) VALUES ($1, $2, $3)", TABLE);
      match client.lock().await.query_one(&query, &[
        &*article_id,
        &reader_id,
        &comment.content,
        ]).await {
